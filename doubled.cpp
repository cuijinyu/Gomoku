#include "doubled.h"
#include "ui_doubled.h"
#include "QMessageBox"
#include<QDialog>

doubled::doubled(QDialog *parent) :QDialog(parent),ui(new Ui::doubled),ai()
{
    ui->setupUi(this);
...
//  <chess board>
void doubled::mousePressEvent(QMouseEvent *m)
{
    if(beginPlay)
    {
        mouse_x = m->pos().x();
...
void doubled::keyPressEvent(QKeyEvent *k)
{
    if(k->key() == Qt::Key_A)
        exit(1);
}
...
void doubled::paintEvent(QPaintEvent *)
{
    if(beginPlay)
    {
        if(chess_color == 1)
...
//  <坐标映射>
int doubled::toRealX(int virtualX)
{
    return 40*virtualX+320;
}

int doubled::toRealY(int virtualY)
{
    return 40*virtualY+100;
}

int doubled::toVirtualX(int realX)
{
    return (realX-320)/40;
}

int doubled::toVirtualY(int realY)
{
    return (realY-100)/40;
}
